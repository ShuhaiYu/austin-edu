export const coursesContent = {
  en: {
    detail: {
      categories: [
        {
          title: "Primary School - Years 1-6",
          subcategories: [
            {
              title: "Enrichment Courses",
              courses: [
                "y1-6-english-enrichment",
                "y1-6-maths-enrichment",
                "y1-6-chinese-enrichment",
                "y3-6-writing",
              ],
            },
            {
              title: "Scholarship / Selective Courses",
              courses: [
                "3-to-5-scholarship-victoria",
                "3-to-5-scholarship-interview-victoria",
                "3-to-5-scholarship-mock-exam-victoria",
                "5-to-7-scholarship-victoria",
                "5-to-7-scholarship-interview-victoria",
                "5-to-7-scholarship-mock-exam-victoria",
                "3-to-5-scholarship-south-australia",
                "5-to-7-scholarship-south-australia",
                "5-to-7-selective-school-south-australia",
              ],
            },
            {
              title: "Competition Courses",
              courses: ["y3-4-amc", "y5-6-amc"],
            },
            {
              title: "Exam Preparation",
              courses: ["y3-y5-naplan", "aeas"],
            },
            {
              title: "Other Courses",
              courses: ["y1-6-extra-holiday-class"],
            },
          ],
        },
        {
          title: "Secondary School - Years 7-9",
          subcategories: [
            {
              title: "Enrichment Courses",
              courses: [
                "y7-9-english",
                "y7-9-maths",
                "y7-9-writing",
                "y8-9-science",
              ],
            },
            {
              title: "Language Courses",
              courses: [
                "y7-9-latin-enrichment",
                "y7-9-france-enrichment",
                "y7-9-japanese-enrichment",
              ],
            },
            {
              title: "Scholarship / Selective Courses",
              courses: [
                "7-to-9-scholarship-victoria",
                "7-to-9-scholarship-interview-victoria",
                "7-to-9-scholarship-mock-exam-victoria",
                "y8-9-selective",
              ],
            },
            {
              title: "Competition Courses",
              courses: ["y7-8-amc"],
            },
            {
              title: "Other Courses",
              courses: ["y7-y9-naplan", "y7-9-extra-holiday-class"],
            },
          ],
        },
        {
          title: "Senior Secondary School - Years 10-12",
          subcategories: [
            {
              title: "Year 10 Pre-VCE Courses",
              courses: [
                "y10-english",
                "y10-eal",
                "y10-maths-advance",
                "y10-maths-standard",
                "y9-10-science",
                "y10-chemistry",
                "y10-latin",
              ],
            },
            {
              title: "VCE English Courses",
              courses: [
                "vce-english-eal-unit1-4",
                "vce-english-language-unit1-4",
              ],
            },
            {
              title: "VCE Maths Courses",
              courses: [
                "vce-maths-methods-unit1-4",
                "vce-specialist-maths-unit1-4",
                "vce-general-maths-unit-1-4",
              ],
            },
            {
              title: "VCE Science Courses",
              courses: [
                "vce-chemistry-unit-1-4",
                "vce-biology-unit-1-4",
                "vce-physics-unit-1-4",
                "vce-psychology-unit-1-4",
              ],
            },
            {
              title: "VCE Languages Courses",
              courses: [
                "vce-lantin-unit-1-4",
                "vce-chinese-first-language-unit-1-4",
                "vce-chinese-second-language-unit-1-4",
                "vce-chinese-second-language-advanced-unit-1-4",
                "vce-japanese-second-language",
              ],
            },
            {
              title: "VCE Business and Economics Courses",
              courses: [
                "vce-accounting-unit-1-4",
                "vce-economics-unit-1-4",
                "vce-business-management-unit-1-4",
                "vce-legal-studies-unit-1-4",
              ],
            },
            {
              title: "VCE Other Courses",
              courses: [
                "vce-vcd-unit-1-4",
                "y10-12-extra-holiday-class",
                "y10-12-mock-exam",
              ],
            },
            {
              title: "IB Courses",
              courses: [
                "ib-english-a-literature-standard-level",
                "ib-english-a-literature-higher-level",
                "ib-english-a-language-literature-standard-level",
                "ib-english-a-language-literature-higher-level",
                "ib-english-b-standard-level",
                "ib-english-b-higher-level",
                "ib-maths-analysis-approaches-standard-level",
                "ib-maths-analysis-approaches-higher-level",
                "ib-chemistry-standard-level",
                "ib-chemistry-higher-level",
                "ib-chinese-a-standard-level",
                "ib-chinese-a-higher-level",
                "ib-chinese-b-standard-level",
                "ib-chinese-b-higher-level",
                "ib-chinese-ab-initio-standard-level",
                "ib-economics-standard-level",
                "ib-economics-higher-level",
              ],
            },
          ],
        },
        {
          title: "Medical Preparation Courses",
          subcategories: [
            {
              title: "Medical Foundation Courses",
              courses: ["ucat", "y12-ucat-medical-interview"],
            },
          ],
        },
        {
          title: "AusYouth Music Courses",
          subcategories: [
            {
              title: "Music Theory & Skills",
              courses: [
                "ameb-theory-music-grade-1-6-term",
                "ameb-theory-music-grade-1-6-holiday",
                "ameb-musicianship-grade-5-6",
                "ameb-aural-sight-reading-grade-4-8",
                "ameb-general-knowledge-coaching",
                "music-appreciation-course",
              ],
            },
            {
              title: "VCE Music Courses",
              courses: [
                "vce-music-performance-unit-1-2",
                "vce-music-performance-unit-3-4",
              ],
            },
            {
              title: "Masterclasses",
              courses: ["ausyouth-music-instrumental-masterclasses"],
            },
          ],
        },
      ],
      courses: {
        "y1-6-english-enrichment": {
          title: "Year 1-6 English Enrichment (Eng/EAL)",
          slug: "y1-6-english-enrichment",
          content: [
            "1.5 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks",
            "30mins-1 hours of weekly homework",
            "12 stage/topic assessments",
            "4 semester assessment exams",
            "1 exam reports",
            "2 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y1-6-maths-enrichment": {
          title: "Year 1-6 Maths Enrichment",
          slug: "y1-6-maths-enrichment",
          content: [
            "1.5 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks each year",
            "1 exclusive Austin Education workbooks",
            "0.5-1 hours of weekly homework",
            "20-40 advanced questions per week",
            "4 term assessments",
            "2 exam reports",
            "1 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y1-6-chinese-enrichment": {
          title: "Year 1-6 Chinese Enrichment",
          slug: "y1-6-chinese-enrichment",
          content: [
            "1.5 hours of weekly small group class sessions",
            "4 exclusive Austin Education textbooks each year",
            "2 hours of weekly homework",
            "30 advanced online questions per week",
            "4 stage/topic assessments",
            "4 term assessments",
            "4 exam reports, including weekly tracking of dictation and homework completion",
            "2 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y3-6-writing": {
          title: "Year 3-6 Writing",
          slug: "y3-6-writing",
          content: [
            "1.5 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks each year",
            "1-2 hours of weekly homework",
            "3 fully reviewed essays per term",
            "12 stage/topic assessments",
            "4 semester assessment exams",
            "2 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "3-to-5-scholarship-victoria": {
          title: "Year 3-to-5 Scholarship Victoria",
          slug: "3-to-5-scholarship-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "3-to-5-scholarship-interview-victoria": {
          title: "Year 3-to-5 Scholarship Interview Victoria",
          slug: "3-to-5-scholarship-interview-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "3-to-5-scholarship-mock-exam-victoria": {
          title: "Year 3-to-5 Scholarship Mock Exam Victoria",
          slug: "3-to-5-scholarship-mock-exam-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "5-to-7-scholarship-victoria": {
          title: "Year 5-to-7 Scholarship Victoria",
          slug: "5-to-7-scholarship-victoria",
          content: [
            "3 hours of weekly class sessions",
            "8 exclusive Austin Education textbooks each year",
            "3 hours of weekly homework",
            "For English: Includes 3-5 fully reviewed essays per term",
            "100 advanced questions per week",
            "20 stage/topic assessments",
            "8 semester assessment exams",
            "20 exclusive Austin Education full-length mock exams",
            "4 exam reports",
            "4 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "5-to-7-scholarship-interview-victoria": {
          title: "Year 5-to-7 Scholarship Interview Victoria",
          slug: "5-to-7-scholarship-interview-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "5-to-7-scholarship-mock-exam-victoria": {
          title: "Year 5-to-7 Scholarship Mock Exam Victoria",
          slug: "5-to-7-scholarship-mock-exam-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y3-4-amc": {
          title: "Year 3-4 AMC ",
          slug: "y3-4-amc",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y5-6-amc": {
          title: "Year 5-6 AMC",
          slug: "y5-6-amc",
          content: [
            "28 lessons, each 2 hours long",
            "1 hour of weekly homework",
            "20 complete practice papers",
            "1000+ practice questions in the question bank",
            "4 full-length mock exams",
            "4 topic tests",
            "2 original practice papers",
            "2 one-on-one post-exam paper analysis sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y3-y5-naplan": {
          title: "Year 3, Year 5 Naplan & holiday program",
          slug: "y3-y5-naplan",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        aeas: {
          title: "AEAS",
          slug: "aeas",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y1-6-extra-holiday-class": {
          title: "Extra holiday class",
          slug: "y1-6-extra-holiday-class",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "3-to-5-scholarship-south-australia": {
          title: "Year 3-to-5 Scholarship South Australia",
          slug: "3-to-5-scholarship-south-australia",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "5-to-7-scholarship-south-australia": {
          title: "Year 5-to-7 Scholarship South Australia",
          slug: "5-to-7-scholarship-south-australia",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "5-to-7-selective-school-south-australia": {
          title: "Year 5-to-7 Selective School South Australia",
          slug: "5-to-7-selective-school-south-australia",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y7-9-english": {
          title: "Year 7-9 English Enrichment （Eng/EAL）",
          slug: "y7-9-english",
          content: [
            "2 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks)",
            "2 hours of weekly homework",
            "2 fully reviewed essays per term",
            "4 semester assessment exams",
            "Exam reports",
            "2 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y7-9-maths": {
          title: "Year 7-9 Maths Enrichment",
          slug: "y7-9-maths",
          content: [
            "2-hour weekly group class",
            "4 exclusive Austin Education textbooks",
            "9 exclusive Austin Education workbooks",
            "1 hour of structured weekly homework",
            "10 optional or extension challenge questions per week",
            "2 unit/topic-based progress test",
            "4 term-level assessment exams",
            "2 detailed exam performance reports",
            "2 parent-teacher meetings per year",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y7-9-writing": {
          title: "Year 7-9 Writing",
          slug: "y7-9-writing",
          content: [
            "1.5 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks",
            "2 hours of weekly homework",
            "3 fully reviewed essays per term",
            "12 stage/topic assessments",
            "4 semester assessment exams",
            "4 exam reports",
            "2 parent-teacher meetings",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y8-9-science": {
          title: "Year 8-9 Science",
          slug: "y8-9-science",
          content: [
            "2 hours of weekly class sessions",
            "4 sets of exclusive Austin Education practice booklets (Lesson Review Questions)",
            "30mins of homework per week",
            "Ample optional/challenge/extension questions for further practice",
            "4 semester exams",
            "4 detailed exam performance reports",
            "Full access to recorded lessons for review",
            "Unlimited academic Q&A support",
            "1-on-1 personalised academic management with a dedicated learning coordinator",
            "Free academic consultation for ongoing progress planning and guidance",
          ],
        },
        "y7-9-latin-enrichment": {
          title: "Year 7-9 Latin Enrichment",
          slug: "y7-9-latin-enrichment",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y7-9-france-enrichment": {
          title: "Year 7-9 France Enrichment",
          slug: "y7-9-france-enrichment",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y7-9-japanese-enrichment": {
          title: "Year 7-9 Japanese Enrichment",
          slug: "y7-9-japanese-enrichment",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "7-to-9-scholarship-victoria": {
          title: "Year 7-to-9 Scholarship Victoria",
          slug: "7-to-9-scholarship-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "7-to-9-scholarship-interview-victoria": {
          title: "Year 7-to-9 Scholarship Interview Victoria",
          slug: "7-to-9-scholarship-interview-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "7-to-9-scholarship-mock-exam-victoria": {
          title: "Year 7-to-9 Scholarship Mock Exam Victoria",
          slug: "7-to-9-scholarship-mock-exam-victoria",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y8-9-selective": {
          title: "Year 8-to-9 Selective School Program",
          slug: "y8-9-selective",
          content: [
            "4 hours of weekly class sessions",
            "6 exclusive Austin Education textbooks",
            "3-4 hours of weekly homework",
            "100 additional/optional/advanced questions per week",
            "6 semester assessment exams",
            "4 exam reports",
            "4 parent-teacher meetings",
            "50 exclusive Austin Education full-length mock exams",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y7-8-amc": {
          title: "Year 7-8 AMC",
          slug: "y7-8-amc",
          content: [
            "28 lessons, each 2 hours long",
            "1 hour of weekly homework",
            "20 complete practice papers",
            "1000+ practice questions in the question bank",
            "4 full-length mock exams",
            "4 topic-based tests",
            "2 original practice papers",
            "2 one-on-one post-exam paper analysis sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y7-y9-naplan": {
          title: "Year 7, Year 9 Naplan",
          slug: "y7-y9-naplan",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y7-9-extra-holiday-class": {
          title: "Extra holiday class",
          slug: "y7-9-extra-holiday-class",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },

        "y10-english": {
          title: "Year 10 Pre-VCE English",
          slug: "y10-english",
          content: [
            "2-hour weekly group class",
            "4 exclusive Austin Education textbooks",
            "4 exclusive Austin Education workbooks",
            "1 hour of structured weekly homework",
            "1 fully marked and personalised essay correction per term",
            "6 in-class assessment tests",
            "2 detailed academic performance reports",
            "Year 10 subject-specific vocabulary list",
            "Recorded lessons",
            "Offline parent-teacher meetings",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y10-eal": {
          title: "Year 10 EAL Foundation Class",
          slug: "y10-eal",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y10-maths-advance": {
          title: "Year 10 Pre-VCE Maths Advance",
          slug: "y10-maths-advance",
          content: [
            "2-hour weekly group class",
            "3 exclusive Austin Education textbooks",
            "3 exclusive Austin Education workbooks",
            "2 hours of structured weekly homework",
            "20+ high-difficulty challenge questions per week",
            "Additional practice tasks provided regularly as needed",
            "4 unit/topic-based progress tests",
            "2 semester-level assessment exams",
            "2 detailed semester academic reports",
            "2 parent-teacher meetings per year",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y10-maths-standard": {
          title: "Year 10 Pre-VCE Maths Standard",
          slug: "y10-maths-standard",
          content: [
            "2 hours of weekly class sessions",
            "1 exclusive Austin Education textbooks",
            "1 exclusive Austin Education workbooks",
            "1 hours of weekly homework",
            "3 stage/assessments topic-based",
            "2 semester assessment exams",
            "4 exam reports",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y9-10-science": {
          title: "Year 9-10 Pre-VCE Science",
          slug: "y9-10-science",
          content: [
            "2 hours of weekly class sessions",
            "4 sets of exclusive Austin Education practice booklets (Lesson Review Questions)",
            "30mins of homework per week",
            "Ample optional/challenge/extension questions for further practice",
            "4 semester exams",
            "4 detailed exam performance reports",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y10-chemistry": {
          title: "Year 10 Pre-VCE Chemistry",
          slug: "y10-chemistry",
          content: [
            "2 hours of weekly class sessions",
            "4 internally curated Austin Education textbooks (theory + key examples + practice)",
            "1 hour of weekly homework",
            "2 in-term assessment exams",
            "1 exclusive full-length Austin mock exam",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "y10-latin": {
          title: "Year 10 Pre-VCE Latin",
          slug: "y10-latin",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "vce-english-eal-unit1-4": {
          title: "VCE English / English as an Additional Language  Unit 1-4",
          slug: "vce-english-eal-unit1-4",
          content: [
            "Unit 1-2：",
            "2-hour weekly group class",
            "4 exclusive Austin Education textbooks",
            "4 exclusive Austin Education workbooks",
            "1 hour of structured weekly homework",
            "1 fully marked and personalised essay correction per term",
            "6 in-class assessment tests per term",
            "2 detailed academic performance reports per term",
            "Year 11 subject-specific vocabulary list",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
            "Unit 3-4：",
            "1.5-hour weekly group class",
            "4 exclusive Austin Education textbooks",
            "4 exclusive Austin Education workbooks",
            "1+ hour of structured weekly homework",
            "2 fully marked and personalised essay corrections per term",
            "5+ full-length Year 12 VCE exam practice sessions",
            "6+ SAC-focused revision sessions",
            "Tailored SAC-preparation materials aligned with individual school preferences and themes",
            "Collection of high-scoring VCE sample essays (45+ English / EAL)",
            "Exclusive EAL listening practice workbook",
            "Austin-designed full-scale mock exams",
            "Complete workbook set for Creating and Presenting Texts",
            "Year 12 subject-specific vocabulary list",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-english-language-unit1-4": {
          title: "VCE English Language Unit 1-4",
          slug: "vce-english-language-unit1-4",
          content: [
            "1.5 hours of weekly class sessions",
            "2 exclusive Austin Education textbooks",
            "2 exclusive Austin Education workbooks",
            "2 hours of weekly homework",
            "5 fully reviewed essays per term",
            "5 stage/topic assessments",
            "2 semester assessment exams",
            "300-word in-class test feedback",
            "2 exam reports",
            "Exclusive Austin Education full-length mock exams",
            "SAC-focused revision",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-maths-methods-unit1-4": {
          title: "VCE Mathematical Methods Unit 1-4",
          slug: "vce-maths-methods-unit1-4",
          content: [
            "2-hour weekly group class",
            "3 exclusive Austin Education textbooks",
            "3 exclusive Austin Education workbooks",
            "2 hours of structured weekly homework",
            "20+ high-difficulty challenge questions per week",
            "Additional practice tasks provided regularly as needed",
            "4 unit/topic-based progress tests (for Units 1&2)",
            "2 semester-level assessment exams (for Units 1&2)",
            "2 detailed semester academic reports",
            "2 parent-teacher meetings per year",
            "5 full-scale Austin mock exams (for Units 3&4)",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-specialist-maths-unit1-4": {
          title: "VCE Specialist Mathematics Unit 1-4",
          slug: "vce-specialist-maths-unit1-4",
          content: [
            "2-hour weekly group class",
            "3 exclusive Austin Education textbooks",
            "3 exclusive Austin Education workbooks",
            "2 hours of structured weekly homework",
            "20+ high-difficulty challenge questions per week",
            "Additional practice tasks provided regularly as needed",
            "4 unit/topic-based progress tests (for Units 1&2)",
            "2 semester-level assessment exams (for Units 1&2)",
            "2 detailed semester academic reports",
            "2 parent-teacher meetings per year",
            "5 full-scale Austin mock exams (for Units 3&4)",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-general-maths-unit-1-4": {
          title: "VCE General Mathematics Unit 1-4",
          slug: "vce-general-maths-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks",
            "1 hours of structured weekly homework",
            "4+ stage/topic assessments",
            "Austin Education full-length mock exams",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-chemistry-unit-1-4": {
          title: "VCE Chemistry Unit 1-4",
          slug: "vce-chemistry-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "4 exclusive Austin Education textbooks",
            "1 hours of weekly homework",
            "2 semester assessment exams",
            "1 exclusive Austin Education full-length mock exams",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-biology-unit-1-4": {
          title: "VCE Biology Unit 1-4",
          slug: "vce-biology-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "3 exclusive Austin Education textbooks",
            "2 hours of weekly homework",
            "4 Mock SACs",
            "2 practice exams",
            "1 exclusive Austin Education full-length mock exams",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-physics-unit-1-4": {
          title: "VCE Physics Unit 1-4",
          slug: "vce-physics-unit-1-4",
          content: [
            "2-hour weekly group class",
            "4 exclusive Austin Education textbooks",
            "8 exclusive Austin Education workbooks",
            "2 hours of structured weekly homework",
            "2 term-level assessment exams",
            "Austin-designed full-length mock exams",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-psychology-unit-1-4": {
          title: "VCE Psychology Unit 1–4",
          slug: "vce-psychology-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "1 hour of weekly homework",
            "1 Area of Study assessment",
            "1 exam report",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-lantin-unit-1-4": {
          title: "VCE Latin Unit 1-4",
          slug: "vce-lantin-unit-1-4",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "vce-chinese-first-language-unit-1-4": {
          title: "VCE Chinese First Language Unit 1–4",
          slug: "vce-chinese-first-language-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "8 exclusive Austin Education textbooks",
            "5 exclusive Austin Education exercise books",
            "1–2 hours of homework per week",
            "50 sets of listening and reading comprehension exercises",
            "1 free online listening practice session per week",
            "5–6 essays per term with detailed corrections and feedback",
            "4 semester exams (Units 1 & 2)",
            "6 full-length Austin mock exams (Units 3 & 4)",
            "10+ one-on-one oral exam mock interviews (Units 3 & 4)",
            "4 one-on-one targeted oral script review sessions (1 hour each)",
            "20 high-quality sample essays and model responses",
            "1 detailed report after each major exam",
            "Austin parent-teacher meeting per term",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-chinese-second-language-unit-1-4": {
          title: "VCE Chinese Second Language Unit 1–4",
          slug: "vce-chinese-second-language-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "8 exclusive Austin Education textbooks",
            "5 exclusive Austin Education exercise books",
            "1–2 hours of homework per week",
            "50 sets of listening and reading comprehension exercises",
            "1 free online listening practice session per week",
            "5–6 essays per term with detailed corrections and feedback",
            "4 semester exams (Units 1 & 2)",
            "6 full-length Austin mock exams (Units 3 & 4)",
            "10+ one-on-one oral exam mock interviews (Units 3 & 4)",
            "4 one-on-one targeted oral script review sessions (1 hour each)",
            "20 high-quality sample essays and model responses",
            "1 detailed report after each major exam",
            "Austin parent-teacher meeting per term",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-chinese-second-language-advanced-unit-1-4": {
          title: "VCE Chinese Second Language Advanced Unit 1–4",
          slug: "vce-chinese-second-language-advanced-unit-1-4",
          content: [
            "2 hours of weekly class sessions",
            "8 exclusive Austin Education textbooks",
            "5 exclusive Austin Education exercise books",
            "1–2 hours of homework per week",
            "50 sets of listening and reading comprehension exercises",
            "1 free online listening practice session per week",
            "5–6 essays per term with detailed corrections and feedback",
            "4 semester exams (Units 1 & 2)",
            "6 full-length Austin mock exams (Units 3 & 4)",
            "10+ one-on-one oral exam mock interviews (Units 3 & 4)",
            "4 one-on-one targeted oral script review sessions (1 hour each)",
            "20 high-quality sample essays and model responses",
            "1 detailed report after each major exam",
            "Austin parent-teacher meeting per term",
            "Targeted SAC revision sessions",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Dedicated 1-on-1 learning manager for personalised issue resolution",
            "Free academic consultation included",
          ],
        },
        "vce-japanese-second-language": {
          title: "VCE Japanese Second Language",
          slug: "vce-japanese-second-language",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "vce-accounting-unit-1-4": {
          title: "VCE Accounting Unit 1-4",
          slug: "vce-accounting-unit-1-4",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "vce-economics-unit-1-4": {
          title: "VCE Economics Unit 1-4",
          slug: "vce-economics-unit-1-4",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "vce-business-management-unit-1-4": {
          title: "VCE Business Management Unit 1-4",
          slug: "vce-business-management-unit-1-4",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "vce-legal-studies-unit-1-4": {
          title: "VCE Legal Studies Unit 1-4",
          slug: "vce-legal-studies-unit-1-4",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "vce-vcd-unit-1-4": {
          title: "VCE Visual Communication Design Unit 1-4",
          slug: "vce-vcd-unit-1-4",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "y10-12-extra-holiday-class": {
          title: "Extra holiday class",
          slug: "y10-12-extra-holiday-class",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "y10-12-mock-exam": {
          title: "Mock Exam",
          slug: "y10-12-mock-exam",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-a-literature-standard-level": {
          title: "IB English A: Literature — Standard Level",
          slug: "ib-english-a-literature-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-a-literature-higher-level": {
          title: "IB English A: Literature — Higher Level",
          slug: "ib-english-a-literature-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-a-language-literature-standard-level": {
          title: "IB English A: Language and Literature — Standard Level",
          slug: "ib-english-a-language-literature-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-a-language-literature-higher-level": {
          title: "IB English A: Language and Literature — Higher Level",
          slug: "ib-english-a-language-literature-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-b-standard-level": {
          title: "IB English B: Standard Level",
          slug: "ib-english-b-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-english-b-higher-level": {
          title: "IB English B: Higher Level",
          slug: "ib-english-b-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-maths-analysis-approaches-standard-level": {
          title:
            "IB Mathematics: Analysis and Approaches (AA) — Standard Level",
          slug: "ib-maths-analysis-approaches-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-maths-analysis-approaches-higher-level": {
          title: "IB Mathematics: Analysis and Approaches (AA) — Higher Level",
          slug: "ib-maths-analysis-approaches-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chemistry-standard-level": {
          title: "IB Chemistry (Sciences) — Standard Level",
          slug: "ib-chemistry-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chemistry-higher-level": {
          title: "IB Chemistry (Sciences) — Higher Level",
          slug: "ib-chemistry-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chinese-a-standard-level": {
          title:
            "IB Chinese A: Literature / Chinese A: Language and Literature — Standard Level",
          slug: "ib-chinese-a-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chinese-a-higher-level": {
          title:
            "IB Chinese A: Literature / Chinese A: Language and Literature — Higher Level",
          slug: "ib-chinese-a-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chinese-b-standard-level": {
          title: "IB Chinese  B: Standard Level",
          slug: "ib-chinese-b-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chinese-b-higher-level": {
          title: "IB Chinese  B: Higher Level",
          slug: "ib-chinese-b-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-chinese-ab-initio-standard-level": {
          title: "IB  Chinese ab initio: Standard Level",
          slug: "ib-chinese-ab-initio-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-economics-standard-level": {
          title: "IB Economics (Individuals and Societies) — Standard Level",
          slug: "ib-economics-standard-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        "ib-economics-higher-level": {
          title: "IB Economics  (Individuals and Societies) — Higher Level",
          slug: "ib-economics-higher-level",
          content: [
            "For details about our term-based courses, please contact us for more information.",
          ],
        },
        ucat: {
          title: "UCAT Preparation Program",
          slug: "ucat",
          content: [
            "2 hours of weekly class sessions",
            "30 exclusive Austin Education workbooks",
            "2+ hours of weekly homework（ depending on stage in course）",
            "5 exam reports",
            "5 exclusive Austin Education full-length mock exams",
            "Recorded lessons",
            "Unlimited Q&A and academic support",
            "Ongoing one-on-one subtest specific performance analysis",
            "Personalised guidance on strengths and weaknesses",
            "Pre-exam debriefing and mindset cultivation",
            "Personalised advice on curating a competitive applicant profile",
            "Dedicated learning consultant for one-on-one issue resolution",
            "Free academic consultation",
          ],
        },
        "y12-ucat-medical-interview": {
          title: "Y12 Medical Interview Preparation Program",
          slug: "y12-ucat-medical-interview",
          content: [
            "For details about our courses, please contact us for more information.",
          ],
        },
        "ameb-theory-music-grade-1-6-term": {
          title: "AMEB Theory of Music Grade 1-6 Term Dates Program",
          slug: "ameb-theory-music-grade-1-6-term",
          content: [
            "Master Your Theory Grade 1-6 by Dulcie Holland – Textbook",
            "Master Your Theory Grade 1-6 by Dulcie Holland – Answer book",
            "Practice Exams – Past exam papers and mock exam papers",
            "Detailed written feedback on each submission",
            "Aural Tests Book (Instrument Grade 4–8)",
            "Sight Reading Books (Instrument-Specific)",
          ],
        },
        "ameb-theory-music-grade-1-6-holiday": {
          title: "AMEB Theory of Music Grade 1-6 Holiday Program",
          slug: "ameb-theory-music-grade-1-6-holiday",
          content: [
            "Master Your Theory Grade 1-5 by Dulcie Holland – Textbook",
            "Master Your Theory Grade 1-5 by Dulcie Holland – Answer book",
            "Practice Exams – Past exam papers and mock exam papers",
            "Detailed written feedback on each submission",
            "Aural Tests Book (Instrument Grade 4–8)",
            "Sight Reading Books (Instrument-Specific)",
          ],
        },
        "ameb-musicianship-grade-5-6": {
          title: "AMEB Musicianship Grade 5/6",
          slug: "ameb-musicianship-grade-5-6",
          content: [
            "AMEB Musicianship Grade 5/6 workbook",
            "Dulcie Holland resources (as support material)",
            "Supplemental exercises and aural practice",
            "15 - 20 weekly lessons, 1 hour per lesson",
            "Theory, harmony, melody writing, and stylistic analysis",
          ],
        },
        "ameb-aural-sight-reading-grade-4-8": {
          title: "AMEB Aural & Sight Reading (Instrument/Voice Grade 4-8)",
          slug: "ameb-aural-sight-reading-grade-4-8",
          content: [
            "AMEB Aural Tests Book (Grades 4–8)",
            "AMEB Sight Reading Books (instrument-specific)",
            "Piano or backing tracks for harmonic support",
            "Total Hours: 10 hours (face-to-face instruction)",
            "Small group or individual lessons",
          ],
        },
        "ameb-general-knowledge-coaching": {
          title:
            "AMEB General Knowledge Coaching for AMEB Grade 8 / Certificate of Performance / AMusA / LMusA",
          slug: "ameb-general-knowledge-coaching",
          content: [
            "Instructor: Dr Sharon Lierse – Internationally Renowned Music Educator",
            "Individualised lessons (1 V 1 online delivery)",
            "Customised worksheets, writing templates, and listening materials",
            "Flexible scheduling based on student need and exam timeline",
            "Repertoire-specific general knowledge preparation",
          ],
        },
        "music-appreciation-course": {
          title: "Music Appreciation Course",
          slug: "music-appreciation-course",
          content: [
            "Instructor: Dr Sharon Lierse – Internationally Renowned Music Educator",
            "10 lessons, 60 minutes each",
            "Small group format (online delivery)",
            "Overview of major music history periods (Baroque to 20th Century)",
            "Writing strategies and terminology usage for high-level exams",
          ],
        },
        "vce-music-performance-unit-1-2": {
          title:
            "VCE Music Repertoire Performance & VCE Music Contemporary Performance Unit 1 & 2 – Written",
          slug: "vce-music-performance-unit-1-2",
          content: [
            "Instructor: Miss Lorraine Zhang",
            "20 lessons total, 60 minutes each",
            "Small group / 1 V 1 online delivery format",
            "Introduction to listening analysis and terminology",
            "Foundation for Unit 3 & 4 success",
          ],
        },
        "vce-music-performance-unit-3-4": {
          title:
            "VCE Music Repertoire Performance & VCE Music Contemporary Performance Unit 3 & 4 – Written",
          slug: "vce-music-performance-unit-3-4",
          content: [
            "Instructor: Mr Gareth De Korte – Head of Music, Melbourne High School",
            "20 x 1-hour lessons across the academic year",
            "Phase 1: SAC Preparation (15 Fortnightly Lessons)",
            "Phase 2: Final Written Exam Support (5 Weekly Lessons)",
            "Performance-driven listening analysis",
          ],
        },
        "ausyouth-music-instrumental-masterclasses": {
          title: "Ausyouth Music Instrumental Masterclasses",
          slug: "ausyouth-music-instrumental-masterclasses",
          content: [
            "Led by Australia's Leading Musicians",
            "Guest artists: Wilma Smith (former MSO Concertmaster) and Rohan De Korte (MSO Cellist)",
            "One-on-One Instruction with personalised feedback",
            "Performance focus with Q&A and career insights",
            "Repertoire & style mastery guidance",
          ],
        },
      },
    },
    content: {
      title: "Well-Structured Course Content",
      subtitle: "Six Core Strengths of Austin Education’s Curriculum",
      description:
        "Austin Education's courses are built on six core principles: comprehensive coverage, appropriate depth, clear objectives, structured progression, real-time updates, and personalised design. These elements work together to support students in making significant academic progress while equipping them for future success.",
      features: [
        "Thorough Coverage: A Complete Learning Experience; Our courses comprehensively address all essential areas of the exam syllabus, ensuring a solid academic foundation. With a structured progression tailored to different learning levels, students can advance with confidence. Real-world applications and interdisciplinary links further enrich their understanding.",
        "In-Depth Learning: Beyond School Standards; Our courses go beyond regular school lessons, offering more advanced content and effective exam techniques. We focus on developing critical thinking and problem-solving skills, with a well-planned structure that helps students improve steadily and gain full mastery of the subject.",
        "Logical and Structured: Step-by-Step Learning; Our courses follow a clear and organised approach, making it easier for students to absorb and apply knowledge. The content smoothly transitions from primary to high school, ensuring continuous learning. A flexible, modular design allows students to focus on specific areas for targeted improvement.",
        "Goal-Oriented: Focused on Future Success; Designed around key exam topics and challenges, our courses provide smart strategies to boost scores. We also include competitions and practical projects to help students apply what they learn. College admissions and career guidance are built into the curriculum, giving students a clear path forward.",
        "Timely Updates: Keeping Pace with Curriculum Developments; Our courses are regularly updated to reflect the latest exam policies and syllabus adjustments, ensuring that students stay focused on key content. Study materials are continuously refined to incorporate current educational trends, while teaching strategies adapt swiftly to changes in exam formats and requirements.",
        "Meeting Diverse Needs: Personalised Learning at the Core; Courses are designed to align with each student’s level and goals, offering tailored learning pathways. Engaging lesson structures spark curiosity and motivation, while appropriately challenging content encourages students to push their limits, fostering a sense of achievement and confidence.",
      ],
      gradeTabs: [
        { value: "y1-y6", label: "Year 1-6" },
        { value: "y7-y9", label: "Year 7-9" },
        { value: "y10-y12", label: "Year 10-12" },
      ],
      gradeContents: [
        {
          value: "y1-y6",
          title: "Building Strong Foundations and Positive Learning Habits",
          description:
            "The primary years are a crucial stage for developing foundational skills, fostering curiosity, and building positive study habits. Austin Education provides structured learning designed to support young learners in developing confidence and a love for learning.",
          contents: [
            {
              title: "Engaging and Interactive Lessons",
              description:
                "Lessons incorporate diverse content and interactive teaching methods to broaden students’ knowledge and enhance their focus and independent learning skills. Course materials extend beyond school curricula, offering students additional challenges to build a strong academic foundation.",
              icon: "book",
              image: "/courses/content/y1-6-1.jpeg",
            },
            {
              title: "Consistent Practice and Reinforcement",
              description:
                "A combination of in-class exercises, paper-based homework, and an online assignment system helps students consolidate their learning, strengthen problem-solving skills, and develop logical thinking.",
              icon: "repeat",
              image: "/courses/content/y1-6-2.jpeg",
            },
            {
              title: "Regular Progress Checks",
              description:
                "End-of-term assessments and structured test practice help familiarise students with exam conditions, developing resilience and preparing them for future academic challenges.",
              icon: "check",
              image: "/courses/content/y1-6-3.jpeg",
            },
            {
              title: "Exam Preparation and Mock Testing",
              description:
                "Structured support is available for key assessments such as NAPLAN, scholarship tests, AEAS, and AMC, helping students refine their exam techniques and build confidence.",
              icon: "test",
              image: "/courses/content/y1-6-4.jpeg",
            },
            {
              title: "Parent–School Communication",
              description:
                "Regular progress reports and parent-teacher meetings provide insights into students’ learning progress, ensuring a supportive partnership in their education journey.",
              icon: "communication",
              image: "/courses/content/y1-6-5.jpeg",
            },
          ],
        },
        {
          value: "y7-y9",
          title:
            "Strengthening Skills and Establishing a Strong Academic Foundation",
          description:
            "The middle school years mark a transition from foundational learning to skill development. During this stage, Austin Education helps students refine their study techniques, deepen their subject knowledge, and build the confidence needed for senior studies.",
          contents: [
            {
              title: "Structured Course Materials",
              description:
                "Our curriculum extends beyond school standards, bridging key content between lower and upper secondary levels to ensure a smooth academic progression.",
              icon: "book",
              image: "/courses/content/y7-9-1.jpeg",
            },
            {
              title: "Experienced Teaching Team",
              description:
                "Teachers focus on fostering subject interest while strengthening core concepts, helping students build the skills required for more advanced study.",
              icon: "chalkboard",
              image: "/courses/content/y7-9-2.jpeg",
            },
            {
              title: "Comprehensive Assignment System",
              description:
                "Carefully designed exercises reinforce classroom learning, enhance problem-solving ability, and encourage independent study habits.",
              icon: "clipboard-list",
              image: "/courses/content/y7-9-3.jpeg",
            },
            {
              title: "Exam Readiness and Performance Training",
              description:
                "End-of-term tests and mock exams develop students’ exam techniques, time management, and ability to perform under pressure.",
              icon: "clock",
              image: "/courses/content/y7-9-4.jpeg",
            },
            {
              title: "Targeted Preparation for Key Assessments",
              description:
                "Support is available for selective entry exams, scholarship tests, NAPLAN, AEAS, AMC, and other academic challenges, helping students work towards their individual goals.",
              icon: "award",
              image: "/courses/content/y7-9-5.jpeg",
            },
            {
              title: "Early Study Planning and Parent Communication",
              description:
                "Regular academic reviews and discussions help families make informed decisions about subject selection and study strategies as students prepare for VCE.",
              icon: "communication",
              image: "/courses/content/y7-9-6.jpeg",
            },
          ],
        },
        {
          value: "y10-y12",
          title: "Maximising VCE Success with Tailored Teaching",
          description:
            "The senior years play a key role in shaping students’ academic outcomes and future pathways. Austin Education provides structured support to help students refine their knowledge, develop effective exam strategies, and work towards achieving strong VCE results.",
          contents: [
            {
              title: "Guided Subject Selection",
              description:
                "Experienced teachers provide insights into subject choices based on students’ interests, strengths, and university aspirations, helping them build a clear academic pathway.",
              icon: "compass",
              image: "/courses/content/y10-12-1.jpeg",
            },
            {
              title: "Specialist Curriculum and Study Plans",
              description:
                " A structured learning approach, developed from years of experience, covering key content, revision cycles, and exam-focused strategies.",
              icon: "file-text",
              image: "/courses/content/y10-12-2.jpeg",
            },
            {
              title: "Personalised Learning Options",
              description:
                "Courses are available in different formats, including group classes, small groups, and one-on-one tutoring, allowing students to find a study approach that best suits their needs.",
              icon: "sliders-horizontal",
              image: "/courses/content/y10-12-3.jpeg",
            },
            {
              title: "Experienced VCE Educators",
              description:
                "Specialist teachers deliver in-depth lessons aligned with exam requirements, focusing on key topics to support learning progression.",
              icon: "users",
              image: "/courses/content/y10-12-4.jpeg",
            },
            {
              title: "Comprehensive Practice and Homework System",
              description:
                "A blend of written exercises and online tools strengthens students’ problem-solving skills and familiarises them with a variety of question types.",
              icon: "clipboard-list",
              image: "/courses/content/y10-12-5.jpeg",
            },
            {
              title: "Benchmark Testing and Performance Analysis",
              description:
                "End-of-term ranking assessments, informed by data insights, help students evaluate their progress and refine their study strategies.",
              icon: "bar-chart-2",
              image: "/courses/content/y10-12-6.jpeg",
            },
            {
              title: "Structured SAC Preparation",
              description:
                "Focused revision and targeted practice for school-based assessments help students build confidence and maintain strong results.",
              icon: "target",
              image: "/courses/content/y10-12-7.jpeg",
            },
            {
              title: "Mock Exams and Intensive Review",
              description:
                "Realistic test simulations assess students’ VCE readiness, followed by structured revision and targeted review sessions to consolidate learning and enhance performance.",
              icon: "list-checks",
              image: "/courses/content/y10-12-8.jpeg",
            },
          ],
        },
      ],
    },
    feature: [
      {
        title: "Exceptional Teaching Team",
        subtitle: "Passionate Educators Who Inspire Learning",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "Teacher Qualifications",
            description:
              "Our team is made up of highly experienced educators from across Australia, many of whom have achieved top academic results or come from prestigious educational backgrounds.",
            icon: "/courses/icons/team icon 1.svg",
          },
          {
            title: "Teaching Style",
            description:
              "Our teachers are skilled at simplifying complex concepts, using humour and interactive methods to engage students and create an enjoyable learning atmosphere.",
            icon: "/courses/icons/team icon 2.svg",
          },
          {
            title: "Personalised Guidance",
            description:
              "We understand that every student is unique, and we tailor our teaching approach to meet their specific needs and pace, ensuring a personalised and effective learning journey.",
            icon: "/courses/icons/team icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-teachers/1.png",
            "/courses/carousel-teachers/2.png",
            "/courses/carousel-teachers/3.png",
            "/courses/carousel-teachers/4.png",
            "/courses/carousel-teachers/5.png",
            "/courses/carousel-teachers/6.png",
            "/courses/carousel-teachers/7.png",
            "/courses/carousel-teachers/8.png",
            "/courses/carousel-teachers/9.png",
            "/courses/carousel-teachers/10.png",
            "/courses/carousel-teachers/11.png",
            "/courses/carousel-teachers/12.png",
            "/courses/carousel-teachers/13.png",
            "/courses/carousel-teachers/14.png",
          ],
        },
      },
      {
        title: "Visible Learning Outcomes",
        subtitle: "Progress You Can Observe and Experience",
        color: "bg-yellow-500/80",
        textColor: "text-yellow-500/80",
        advantages: [
          {
            title: "Significant Score Improvement",
            description:
              "Our data speaks for itself, as our courses have helped numerous students achieve breakthroughs in key exams like VCE, NAPLAN, and scholarship tests.",
            icon: "/courses/icons/outcome icon 1.svg",
          },
          {
            title: "Comprehensive Skill Enhancement",
            description:
              "By focusing on problem-solving techniques, time management, critical thinking, and other essential skills, we help students gain long-term competitive advantages.",
            icon: "/courses/icons/outcome icon 2.svg",
          },
          {
            title: "Boosted Learning Confidence",
            description:
              "Beyond academic achievement, our courses also help students build confidence and a positive mindset, instilling a genuine passion and eagerness for learning.",
            icon: "/courses/icons/outcome icon 3.svg",
          },
        ],
        button: {
          text: "Learn More About Our Achievements",
          link: "/achievements",
          fromColor: "from-yellow-500",
          toColor: "to-yellow-700",
        },
      },
      {
        title: "An Effective Teaching Methodology",
        subtitle: "Combining Structure with Adaptability",
        color: "bg-red-700/80",
        textColor: "text-red-700/80",
        advantages: [
          {
            title: "Course Design",
            description:
              "Our courses are carefully structured to progress from foundational to advanced levels, ensuring students build a solid knowledge base step by step and preventing any gaps in learning.",
            icon: "/courses/icons/meth icon 1.svg",
          },
          {
            title: "Diverse Teaching Formats",
            description:
              "We offer in-person classes, interactive online lessons, and blended learning options to cater to different learning needs, providing flexibility while maintaining efficiency.",
            icon: "/courses/icons/meth icon 2.svg",
          },
          {
            title: "High Interaction",
            description:
              "Our classrooms are designed to engage students with frequent questions, interactive discussions, practice exercises, and feedback sessions, ensuring deep understanding and enhancing the overall learning experience.",
            icon: "/courses/icons/meth icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-1/1.png",
            "/courses/carousel-1/2.png",
            "/courses/carousel-1/3.png",
            "/courses/carousel-1/4.png",
            "/courses/carousel-1/5.png",
            "/courses/carousel-1/6.png",
            "/courses/carousel-1/7.png",
            "/courses/carousel-1/8.png",
            "/courses/carousel-1/9.png",
            "/courses/carousel-1/10.png",
          ],
        },
      },
      {
        title: "Comprehensive Resource Support",
        subtitle: "Empowering Every Step of the Learning Journey",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "High‑Quality Internal Materials",
            description:
              "We've developed exclusive, high-quality resources that cover all key exam topics and offer abundant supplementary learning materials.",
            icon: "/courses/icons/support icon 1.svg",
          },
          {
            title: "Assignments and Practice",
            description:
              "Our courses include a sufficient and diverse range of homework and exercises, designed to reinforce knowledge and enhance problem-solving abilities, providing full support for students' learning progress.",
            icon: "/courses/icons/support icon 2.svg",
          },
          {
            title: "Periodic Assessments",
            description:
              "Regular evaluations and full-scale mock exams allow students to track their learning progress, adjust strategies, and steadily improve their performance.",
            icon: "/courses/icons/support icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-2/1.png",
            "/courses/carousel-2/2.png",
            "/courses/carousel-2/3.png",
            "/courses/carousel-2/4.png",
            "/courses/carousel-2/5.png",
            "/courses/carousel-2/6.png",
            "/courses/carousel-2/7.png",
          ],
        },
      },
      {
        title: "High Satisfaction from Students and Parents",
        subtitle: "Efficient Learning and Rich Rewards",
        color: "bg-yellow-500/80",
        textColor: "text-yellow-500/80",
        advantages: [
          {
            title: "Exceptional Student Experience",
            description:
              "Students consistently express that the courses are engaging and efficient, offering both a sense of academic achievement and the enjoyment of exploring new knowledge.",
            icon: "/courses/icons/sat icon 1.svg",
          },
          {
            title: "Close Parent–School Communication",
            description:
              "Through regular progress reports and parent-teacher meetings, we provide prompt feedback on students' learning progress and maintain close communication with parents to collaboratively optimise learning plans.",
            icon: "/courses/icons/sat icon 2.svg",
          },
          {
            title: "Outstanding Social Reputation",
            description:
              "With years of successful case studies, we have earned widespread recognition and strong support from students, parents, and the wider community.",
            icon: "/courses/icons/sat icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-3/1.png",
            "/courses/carousel-3/2.png",
            "/courses/carousel-3/3.png",
            "/courses/carousel-3/4.png",
            "/courses/carousel-3/5.png",
            "/courses/carousel-3/6.png",
            "/courses/carousel-3/7.png",
          ],
        },
      },
      {
        title: "Comprehensive Support",
        subtitle: "More Than Just Classes",
        color: "bg-red-700/80",
        textColor: "text-red-700/80",
        advantages: [
          {
            title: "All‑Round Service",
            description:
              "From course planning to academic and psychological support, we cover all aspects of a student's learning.",
            icon: "/courses/icons/com icon 1.svg",
          },
          {
            title: "Flexible & Personalised",
            description:
              "Tailored course plans to fit schedules and goals, maximising learning efficiency.",
            icon: "/courses/icons/com icon 2.svg",
          },
          {
            title: "Commitment to Results",
            description:
              "Offering fair refund or compensation policies as a sign of our confidence in course effectiveness and responsibility for student outcomes.",
            icon: "/courses/icons/com icon 3.svg",
          },
        ],
        button: {
          text: "Learn More About Our Support",
          link: "/resource_hub",
          fromColor: "from-red-500",
          toColor: "to-red-700",
        },
      },
      {
        title: "Exceptional Value",
        subtitle: "Rich Resources, Beyond Expectations",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "Wide Range of Resources",
            description:
              "Students receive excellent teaching, extensive learning materials, and full support services, all at a competitive price.",
            icon: "/courses/icons/value icon 1.svg",
          },
          {
            title: "Extra Benefits",
            description:
              "We provide additional services such as course planning, university guidance, and nurturing subject interests, ensuring students are well-prepared for their academic and career future.",
            icon: "/courses/icons/value icon 2.svg",
          },
        ],
        button: {
          text: "Learn More",
          link: "/contact_us",
          fromColor: "from-blue-500",
          toColor: "to-blue-700",
        },
      },
    ],
  },
  zh: {
    detail: {
      categories: [
        {
          title: "小学 1-6 年级",
          subcategories: [
            {
              title: "培优课程",
              courses: [
                "y1-6-english-enrichment",
                "y1-6-maths-enrichment",
                "y1-6-chinese-enrichment",
                "y3-6-writing",
              ],
            },
            {
              title: "奖学金/精英公校课程",
              courses: [
                "3-to-5-scholarship-victoria",
                "3-to-5-scholarship-interview-victoria",
                "3-to-5-scholarship-mock-exam-victoria",
                "5-to-7-scholarship-victoria",
                "5-to-7-scholarship-interview-victoria",
                "5-to-7-scholarship-mock-exam-victoria",
                "3-to-5-scholarship-south-australia",
                "5-to-7-scholarship-south-australia",
                "5-to-7-selective-school-south-australia",
              ],
            },
            {
              title: "竞赛课程",
              courses: ["y3-4-amc", "y5-6-amc"],
            },
            {
              title: "考试准备",
              courses: ["y3-y5-naplan", "aeas"],
            },
            {
              title: "其他课程",
              courses: ["y1-6-extra-holiday-class"],
            },
          ],
        },
        {
          title: "初中 7-9 年级",
          subcategories: [
            {
              title: "培优课程",
              courses: [
                "y7-9-english",
                "y7-9-maths",
                "y7-9-writing",
                "y8-9-science",
              ],
            },
            {
              title: "语言课程",
              courses: [
                "y7-9-latin-enrichment",
                "y7-9-france-enrichment",
                "y7-9-japanese-enrichment",
              ],
            },
            {
              title: "奖学金/精英公校课程",
              courses: [
                "7-to-9-scholarship-victoria",
                "7-to-9-scholarship-interview-victoria",
                "7-to-9-scholarship-mock-exam-victoria",
                "y8-9-selective",
              ],
            },
            {
              title: "竞赛课程",
              courses: ["y7-8-amc"],
            },
            {
              title: "其他课程",
              courses: ["y7-y9-naplan", "y7-9-extra-holiday-class"],
            },
          ],
        },
        {
          title: "高中 10-12 年级",
          subcategories: [
            {
              title: "10年级 VCE衔接课程",
              courses: [
                "y10-english",
                "y10-eal",
                "y10-maths-advance",
                "y10-maths-standard",
                "y9-10-science",
                "y10-chemistry",
                "y10-latin",
              ],
            },
            {
              title: "VCE 英文课程",
              courses: [
                "vce-english-eal-unit1-4",
                "vce-english-language-unit1-4",
              ],
            },
            {
              title: "VCE 数学课程",
              courses: [
                "vce-maths-methods-unit1-4",
                "vce-specialist-maths-unit1-4",
                "vce-general-maths-unit-1-4",
              ],
            },
            {
              title: "VCE 科学课程",
              courses: [
                "vce-chemistry-unit-1-4",
                "vce-biology-unit-1-4",
                "vce-physics-unit-1-4",
                "vce-psychology-unit-1-4",
              ],
            },
            {
              title: "VCE 语言课程",
              courses: [
                "vce-lantin-unit-1-4",
                "vce-chinese-first-language-unit-1-4",
                "vce-chinese-second-language-unit-1-4",
                "vce-chinese-second-language-advanced-unit-1-4",
                "vce-japanese-second-language",
              ],
            },
            {
              title: "VCE 商业与经济课程",
              courses: [
                "vce-accounting-unit-1-4",
                "vce-economics-unit-1-4",
                "vce-business-management-unit-1-4",
                "vce-legal-studies-unit-1-4",
              ],
            },
            {
              title: "VCE 其他课程",
              courses: [
                "vce-vcd-unit-1-4",
                "y10-12-extra-holiday-class",
                "y10-12-mock-exam",
              ],
            },
            {
              title: "IB 课程",
              courses: [
                "ib-english-a-literature-standard-level",
                "ib-english-a-literature-higher-level",
                "ib-english-a-language-literature-standard-level",
                "ib-english-a-language-literature-higher-level",
                "ib-english-b-standard-level",
                "ib-english-b-higher-level",
                "ib-maths-analysis-approaches-standard-level",
                "ib-maths-analysis-approaches-higher-level",
                "ib-chemistry-standard-level",
                "ib-chemistry-higher-level",
                "ib-chinese-a-standard-level",
                "ib-chinese-a-higher-level",
                "ib-chinese-b-standard-level",
                "ib-chinese-b-higher-level",
                "ib-chinese-ab-initio-standard-level",
                "ib-economics-standard-level",
                "ib-economics-higher-level",
              ],
            },
          ],
        },
        {
          title: "医学备考班",
          subcategories: [
            {
              title: "医学预科课程",
              courses: ["ucat", "y12-ucat-medical-interview"],
            },
          ],
        },
        {
          title: "Ausyouth Music 课程",
          subcategories: [
            {
              title: "乐理与技能课程",
              courses: [
                "ameb-theory-music-grade-1-6-term",
                "ameb-theory-music-grade-1-6-holiday",
                "ameb-musicianship-grade-5-6",
                "ameb-aural-sight-reading-grade-4-8",
                "ameb-general-knowledge-coaching",
                "music-appreciation-course",
              ],
            },
            {
              title: "VCE 音乐课程",
              courses: [
                "vce-music-performance-unit-1-2",
                "vce-music-performance-unit-3-4",
              ],
            },
            {
              title: "大师课",
              courses: ["ausyouth-music-instrumental-masterclasses"],
            },
          ],
        },
      ],
      courses: {
        "y1-6-english-enrichment": {
          title: "1-6年级 英文培优班 (Eng/EAL)",
          slug: "y1-6-english-enrichment",
          content: [
            "每周1.5小时班课",
            "4本澳升独家教材",
            "每周30分钟-1小时课后作业",
            "12次阶段/主题测试",
            "4次学期检测考试",
            "考试成绩报告单",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y1-6-maths-enrichment": {
          title: "1-6年级 数学培优班",
          slug: "y1-6-maths-enrichment",
          content: [
            "每周1.5小时班课",
            "每年4本澳升独家教材",
            "1本澳升独家习题册",
            "每周30分钟-1小时课后作业",
            "每周20-40题高阶题",
            "4次学期检测考试",
            "2份考试成绩报告单",
            "1次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y1-6-chinese-enrichment": {
          title: "1-6年级 中文培优班",
          slug: "y1-6-chinese-enrichment",
          content: [
            "每周1.5小时班课",
            "每年4本澳升独家教材",
            "4本澳升独家习题册",
            "每周2小时课后作业",
            "每周30题额外复习高阶题",
            "4次阶段/主题测试",
            "4次学期检测考试",
            "4份考试成绩报告单，包含每周记录听写及作业完成情况",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y3-6-writing": {
          title: "3-6年级 写作班",
          slug: "y3-6-writing",
          content: [
            "每周1.5小时班课",
            "每年4本澳升独家教材",
            "每周1-2小时课后作业",
            "每学期3篇作文精改",
            "12次阶段/主题测试",
            "4次学期检测考试",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "3-to-5-scholarship-victoria": {
          title: "3考5奖学金班（维洲）",
          slug: "3-to-5-scholarship-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "3-to-5-scholarship-interview-victoria": {
          title: "3考5奖学金面试班（维洲）",
          slug: "3-to-5-scholarship-interview-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "3-to-5-scholarship-mock-exam-victoria": {
          title: "3考5奖学金模拟考试（维洲）",
          slug: "3-to-5-scholarship-mock-exam-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "5-to-7-scholarship-victoria": {
          title: "5考7奖学金班（维洲）",
          slug: "5-to-7-scholarship-victoria",
          content: [
            "每周3小时班课",
            "8本澳升独家教材",
            "每周3小时课后作业",
            "英文包含每学期3-5篇作文精改",
            "20次阶段/主题测试",
            "8次学期检测考试",
            "4次考试成绩报告单",
            "4次家长会",
            "20次澳升独家全真模拟考试",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "5-to-7-scholarship-interview-victoria": {
          title: "5考7奖学金面试班（维洲）",
          slug: "5-to-7-scholarship-interview-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "5-to-7-scholarship-mock-exam-victoria": {
          title: "5考7奖学金模拟考试（维洲）",
          slug: "5-to-7-scholarship-mock-exam-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y3-4-amc": {
          title: "3-4年级 AMC竞赛班",
          slug: "y3-4-amc",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y5-6-amc": {
          title: "5-6年级 AMC竞赛班",
          slug: "y5-6-amc",
          content: [
            "共28节课，每周2小时课程",
            "每周1小时课后作业",
            "20套完整的练习试卷",
            "1000题海量练习题库",
            "4次全真模考",
            "4次阶段/主题测试",
            "2份原创试卷",
            "2次一对一考后试卷分析",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y3-y5-naplan": {
          title: "3年级、5年级 Naplan考试刷题/冲刺班",
          slug: "y3-y5-naplan",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        aeas: {
          title: "AEAS备考",
          slug: "aeas",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y1-6-extra-holiday-class": {
          title: "假期班",
          slug: "y1-6-extra-holiday-class",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "3-to-5-scholarship-south-australia": {
          title: "3考5奖学金班（南澳）",
          slug: "3-to-5-scholarship-south-australia",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "5-to-7-scholarship-south-australia": {
          title: "5考7奖学金班（南澳）",
          slug: "5-to-7-scholarship-south-australia",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "5-to-7-selective-school-south-australia": {
          title: "5考7精英公校班（南澳）",
          slug: "5-to-7-selective-school-south-australia",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y7-9-english": {
          title: "7-9年级 英文培优班（Eng/EAL）",
          slug: "y7-9-english",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "每周2小时课后作业",
            "一个学期2篇作文精改",
            "4次学期检测考试",
            "考试成绩汇报单",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y7-9-maths": {
          title: "7-9年级 数学培优班",
          slug: "y7-9-maths",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "9本澳升独家习题册",
            "每周1小时课后作业",
            "每周10题额外高阶题",
            "2次阶段/主题测试",
            "4次学期检测考试",
            "2次考试成绩汇报单",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y7-9-writing": {
          title: "7-9年级 写作班",
          slug: "y7-9-writing",
          content: [
            "每周1.5小时班课",
            "4本澳升独家教材",
            "每周2小时课后作业",
            "每学期3篇作文精改",
            "12次阶段/主题测试",
            "4次学期检测考试",
            "4份考试成绩汇报单",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y8-9-science": {
          title: "8-9年级 科学培优班",
          slug: "y8-9-science",
          content: [
            "每周2小时班课",
            "4本澳升独家配套习题",
            "每周30分钟课后作业",
            "充足额外练习题",
            "4次学期检测考试",
            "4份考试成绩汇报单",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y7-9-latin-enrichment": {
          title: "7-9年级拉丁初级班",
          slug: "y7-9-latin-enrichment",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y7-9-france-enrichment": {
          title: "7-9年级法语初级班",
          slug: "y7-9-france-enrichment",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y7-9-japanese-enrichment": {
          title: "7-9年级日语初级班",
          slug: "y7-9-japanese-enrichment",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "7-to-9-scholarship-victoria": {
          title: "7考9奖学金班（维洲）",
          slug: "7-to-9-scholarship-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "7-to-9-scholarship-interview-victoria": {
          title: "7考9奖学金面试班（维洲）",
          slug: "7-to-9-scholarship-interview-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "7-to-9-scholarship-mock-exam-victoria": {
          title: "7考9奖学金模拟考试（维洲）",
          slug: "7-to-9-scholarship-mock-exam-victoria",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y8-9-selective": {
          title: "8考9精英公校班（维洲）",
          slug: "y8-9-selective",
          content: [
            "每周4小时班课",
            "6本澳升独家教材",
            "每周3-4小时课后作业",
            "每周100题额外高阶题",
            "6次学期检测考试",
            "4份考试成绩汇报单",
            "4次家长会",
            "50次澳升独家全真模拟考试",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y7-8-amc": {
          title: "7-8年级 AMC竞赛班",
          slug: "y7-8-amc",
          content: [
            "共28节课，每周2小时课程",
            "每周1小时课后作业",
            "20套完整的练习试卷",
            "1000题海量练习题库",
            "4次全真模考",
            "4次阶段/主题测试",
            "2份原创试卷",
            "2次一对一考后试卷分析",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y7-y9-naplan": {
          title: "7年级、9年级 Naplan考试刷题/冲刺班",
          slug: "y7-y9-naplan",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y7-9-extra-holiday-class": {
          title: "假期班",
          slug: "y7-9-extra-holiday-class",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },

        "y10-english": {
          title: "10年级英文进阶班 · VCE衔接",
          slug: "y10-english",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "4本澳升独家习题册",
            "每周1小时课后作业",
            "每学期1篇作文精改",
            "6次学期检测考试",
            "2份详细考试成绩汇报单",
            "10年级专项单词表",
            "课程录像",
            "线下家长会",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y10-eal": {
          title: "10年级EAL进阶班",
          slug: "y10-eal",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y10-maths-advance": {
          title: "10年级数学精英班 · VCE衔接",
          slug: "y10-maths-advance",
          content: [
            "每周2小时班课",
            "3本澳升独家教材",
            "3本澳升独家习题册",
            "每周2小时课后作业",
            "每周20+题拔高题",
            "不定期充足额外练习",
            "4次阶段/主题测试",
            "2次学期检测考试",
            "2次学期成绩汇报单",
            "2次家长会",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y10-maths-standard": {
          title: "10年级数学进阶班 · VCE衔接",
          slug: "y10-maths-standard",
          content: [
            "每周2小时班课",
            "1本澳升独家教材",
            "1本澳升独家习题册",
            "每周1小时课后作业",
            "3次阶段/主题测试",
            "2次学期检测考试",
            "4次学期成绩汇报单",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y9-10-science": {
          title: "9-10年级科学进阶班 · VCE衔接",
          slug: "y9-10-science",
          content: [
            "每周2小时班课",
            "4本澳升独家配套习题",
            "每周30分钟课后作业",
            "不定期充足额外练习",
            "4次学期检测考试",
            "4次学期成绩汇报单",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y10-chemistry": {
          title: "10年级化学进阶班 · VCE衔接",
          slug: "y10-chemistry",
          content: [
            "每周2小时班课",
            "4本澳升内部精编教材（知识讲解+重点例题+练习）",
            "每周1小时课后作业",
            "2次学期检测考试",
            "1次澳升独家全真模拟考试",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "y10-latin": {
          title: "10年级拉丁进阶班 · VCE衔接",
          slug: "y10-latin",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-english-eal-unit1-4": {
          title: "VCE 英语/英语第二语言  Unit 1-4",
          slug: "vce-english-eal-unit1-4",
          content: [
            "Unit 1-2：",
            "每周2小时班课",
            "4本澳升独家教材",
            "4本澳升独家习题册",
            "每周1小时课后作业",
            "每学期1篇作文精改",
            "6次学期检测考试",
            "2份详细考试成绩汇报单",
            "11年级专项单词表",
            "课程录像",
            "线下家长会",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
            "Unit 3-4：",
            "每周1.5小时班课",
            "4本澳升独家教材",
            "4本澳升独家习题册",
            "每周1小时课后作业",
            "每学期2篇作文精改",
            "5次以上12年级高考模考卷刷卷",
            "6次以上SAC 专项复习",
            "针对日校SAC偏好和话题的独家教材",
            "历年VCE 45+ 高分范文集",
            "EAL独家听力练习册",
            "澳升独家全真模拟考试",
            "Creating A Text 全套练习册",
            "12年级专项单词表",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-english-language-unit1-4": {
          title: "VCE 英语语言 English Language Unit 1-4",
          slug: "vce-english-language-unit1-4",
          content: [
            "每周1.5小时班课",
            "2本澳升独家教材",
            "2本澳升独家习题册",
            "每周2小时课后作业",
            "每学期5篇作文精改",
            "5次阶段/主题测试",
            "2次学期检测考试",
            "300字课堂测试点评",
            "2次考试成绩汇报单",
            "澳升独家全真模拟考试",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-maths-methods-unit1-4": {
          title: "VCE 中数MM Unit1-4",
          slug: "vce-maths-methods-unit1-4",
          content: [
            "每周2小时班课",
            "3本澳升独家教材",
            "3本澳升独家习题册",
            "每周2小时课后作业",
            "每周20+题高阶题",
            "不定期充足额外练习",
            "4次阶段/主题测试（Unit 1&2）",
            "2次学期检测考试( Unit 1&2）",
            "2次学期成绩汇报单",
            "2次家长会",
            "5次澳升独家全真模拟考试（Unit3&4）",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-specialist-maths-unit1-4": {
          title: "VCE 高数SM Unit1-4",
          slug: "vce-specialist-maths-unit1-4",
          content: [
            "每周2小时班课",
            "3本澳升独家教材",
            "3本澳升独家习题册",
            "每周2小时课后作业",
            "每周20+题高阶题",
            "不定期充足额外练习",
            "4次阶段/主题测试（Unit 1&2）",
            "2次学期检测考试( Unit 1&2）",
            "2次学期成绩汇报单",
            "2次家长会",
            "5次澳升独家全真模拟考试（Unit3&4）",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-general-maths-unit-1-4": {
          title: "VCE 低数GM Unit1-4",
          slug: "vce-general-maths-unit-1-4",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "每周1小时课后作业",
            "4+次阶段/主题测试",
            "澳升独家全真模拟考试",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-chemistry-unit-1-4": {
          title: "VCE 化学 Unit1-4",
          slug: "vce-chemistry-unit-1-4",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "每周1小时课后作业",
            "2次学期检测考试",
            "1次澳升独家全真模拟考试",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-biology-unit-1-4": {
          title: "VCE 生物 Unit1-4",
          slug: "vce-biology-unit-1-4",
          content: [
            "每周2小时班课",
            "3本澳升独家教材",
            "每周2小时课后作业",
            "4次模拟SAC测试",
            "2套阶段性练习试卷",
            "1套澳升独家全真模拟考试",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-physics-unit-1-4": {
          title: "VCE 物理 Unit1-4",
          slug: "vce-physics-unit-1-4",
          content: [
            "每周2小时班课",
            "4本澳升独家教材",
            "8本澳升独家习题册",
            "每周2小时课后作业",
            "2次学期检测考试",
            "澳升独家全真模拟考试",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-psychology-unit-1-4": {
          title: "VCE 心理学 Unit 1–4",
          slug: "vce-psychology-unit-1-4",
          content: [
            "每周2小时班课",
            "每周1小时课后作业",
            "1次阶段检测考试",
            "1次考试成绩汇报单",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-lantin-unit-1-4": {
          title: "VCE 拉丁 Unit 1-4",
          slug: "vce-lantin-unit-1-4",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-chinese-first-language-unit-1-4": {
          title: "VCE 中文第一语言 Unit 1–4",
          slug: "vce-chinese-first-language-unit-1-4",
          content: [
            "每周2小时班课",
            "8本澳升独家教材",
            "5本澳升独家习题册",
            "每周1-2小时课后作业",
            "50套听力阅读练习",
            "每周1次免费线上听力练习",
            "每学期5-6篇作文精改",
            "4次学期检测考试（unit 1&2）",
            "6次澳升独家全真模拟笔试考试 （unit 3&4）",
            "10+澳升一对一口语模考模拟练习（unit 3&4）",
            "4次澳升口语稿一对一单项针对性修改（每次一小时）",
            "20篇高质量范文",
            "考试成绩汇报单",
            "澳升家长会",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-chinese-second-language-unit-1-4": {
          title: "VCE 中文第二语言 Unit 1–4",
          slug: "vce-chinese-second-language-unit-1-4",
          content: [
            "每周2小时班课",
            "8本澳升独家教材",
            "5本澳升独家习题册",
            "每周1-2小时课后作业",
            "50套听力阅读练习",
            "每周1次免费线上听力练习",
            "每学期5-6篇作文精改",
            "4次学期检测考试（unit 1&2）",
            "6次澳升独家全真模拟笔试考试 （unit 3&4）",
            "10+澳升一对一口语模考模拟练习（unit 3&4）",
            "4次澳升口语稿一对一单项针对性修改（每次一小时）",
            "20篇高质量范文",
            "考试成绩汇报单",
            "澳升家长会",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-chinese-second-language-advanced-unit-1-4": {
          title: "VCE 中文第二语言高级组 Unit 1–4",
          slug: "vce-chinese-second-language-advanced-unit-1-4",
          content: [
            "每周2小时班课",
            "8本澳升独家教材",
            "5本澳升独家习题册",
            "每周1-2小时课后作业",
            "50套听力阅读练习",
            "每周1次免费线上听力练习",
            "每学期5-6篇作文精改",
            "4次学期检测考试（unit 1&2）",
            "6次澳升独家全真模拟笔试考试 （unit 3&4）",
            "10+澳升一对一口语模考模拟练习（unit 3&4）",
            "4次澳升口语稿一对一单项针对性修改（每次一小时）",
            "20篇高质量范文",
            "考试成绩汇报单",
            "澳升家长会",
            "SAC 专项复习",
            "课程录像",
            "无限的沟通答疑",
            "私人学管一对一解决问题",
            "免费学业咨询",
          ],
        },
        "vce-japanese-second-language": {
          title: "VCE 日语第二语言 ",
          slug: "vce-japanese-second-language",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-accounting-unit-1-4": {
          title: "VCE 会计 Unit 1-4",
          slug: "vce-accounting-unit-1-4",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-economics-unit-1-4": {
          title: "VCE 经济 Unit 1-4",
          slug: "vce-economics-unit-1-4",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-business-management-unit-1-4": {
          title: "VCE 工商管理 Unit 1-4",
          slug: "vce-business-management-unit-1-4",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-legal-studies-unit-1-4": {
          title: "VCE 法律研究 Unit 1-4",
          slug: "vce-legal-studies-unit-1-4",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "vce-vcd-unit-1-4": {
          title: "VCE 视觉传达设计VCD Unit 1-4",
          slug: "vce-vcd-unit-1-4",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "y10-12-extra-holiday-class": {
          title: "假期班",
          slug: "y10-12-extra-holiday-class",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "y10-12-mock-exam": {
          title: "模拟考试",
          slug: "y10-12-mock-exam",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-a-literature-standard-level": {
          title: "IB 英语 A：文学 — 标准水平",
          slug: "ib-english-a-literature-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-a-literature-higher-level": {
          title: "IB 英语 A：文学 — 高级水平",
          slug: "ib-english-a-literature-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-a-language-literature-standard-level": {
          title: "IB 英语 A：语言与文学 — 标准水平",
          slug: "ib-english-a-language-literature-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-a-language-literature-higher-level": {
          title: "IB 英语 A：语言与文学 — 高级水平",
          slug: "ib-english-a-language-literature-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-b-standard-level": {
          title: "IB 英语 B — 标准水平",
          slug: "ib-english-b-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-english-b-higher-level": {
          title: "IB 英语 B — 高级水平",
          slug: "ib-english-b-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-maths-analysis-approaches-standard-level": {
          title: "IB 数学：分析与方法（AA） — 标准水平",
          slug: "ib-maths-analysis-approaches-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-maths-analysis-approaches-higher-level": {
          title: "IB 数学：分析与方法 (AA) — 高级水平",
          slug: "ib-maths-analysis-approaches-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chemistry-standard-level": {
          title: "IB 化学 — 标准水平",
          slug: "ib-chemistry-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chemistry-higher-level": {
          title: "IB 化学 — 高级水平",
          slug: "ib-chemistry-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chinese-a-standard-level": {
          title: "IB 中文 A：文学 / 中文 A：语言与文学 — 标准水平",
          slug: "ib-chinese-a-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chinese-a-higher-level": {
          title: "IB 中文 A：文学 / 中文 A：语言与文学 — 高级水平",
          slug: "ib-chinese-a-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chinese-b-standard-level": {
          title: "IB 中文 B — 标准水平",
          slug: "ib-chinese-b-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chinese-b-higher-level": {
          title: "IB 中文 B — 高级水平",
          slug: "ib-chinese-b-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-chinese-ab-initio-standard-level": {
          title: "IB 中文初级课程（ab initio）— 标准水平",
          slug: "ib-chinese-ab-initio-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-economics-standard-level": {
          title: "IB 经济学 — 标准水平",
          slug: "ib-economics-standard-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        "ib-economics-higher-level": {
          title: "IB 经济学 — 高级水平",
          slug: "ib-economics-higher-level",
          content: ["关于阶段性课程详情，欢迎联系我们获取更多信息。"],
        },
        ucat: {
          title: "UCAT医学预科课程（Y10–Y12）",
          slug: "ucat",
          content: [
            "每周2小时正课授课",
            "30本澳升教育独家练习册",
            "每周2小时以上课后作业（根据课程进度阶段调整）",
            "5次考试成绩报告分析",
            "5次澳升独家全真模考",
            "课程录像",
            "无限次提问与学术支持",
            "持续进行一对一子项目专项表现分析",
            "针对学生强项与弱项提供个性化学习指导",
            "考前心理辅导与心态建设",
            "提供打造高竞争力申请人背景的个性化建议",
            "专属学管老师一对一解决问题",
            "免费学业咨询服务",
          ],
        },
        "y12-ucat-medical-interview": {
          title: "12年级医学面试辅导",
          slug: "y12-ucat-medical-interview",
          content: ["关于课程详情，欢迎联系我们获取更多信息。"],
        },
        "ameb-theory-music-grade-1-6-term": {
          title: "AMEB 乐理学期班 Grade 1–6",
          slug: "ameb-theory-music-grade-1-6-term",
          content: [
            "《Master Your Theory》1-6级（作者：Dulcie Holland）– 教材",
            "《Master Your Theory》1-6级（作者：Dulcie Holland）– 答案册",
            "模拟试卷与历年真题 – 提供备考练习和自测",
            "每份作业附有详细书面反馈",
            "试唱训练教材（适用于乐器考试4–8级）",
            "视奏教材（根据不同乐器定制）",
          ],
        },
        "ameb-theory-music-grade-1-6-holiday": {
          title: "AMEB 乐理假期班 Grade 1–6",
          slug: "ameb-theory-music-grade-1-6-holiday",
          content: [
            "《Master Your Theory》1-5级（作者：Dulcie Holland）– 教材",
            "《Master Your Theory》1-5级（作者：Dulcie Holland）– 答案册",
            "模拟试卷与历年真题 – 提供备考练习和自测",
            "每份作业附有详细书面反馈",
            "试唱训练教材（适用于乐器考试4–8级）",
            "视奏教材（根据不同乐器定制）",
          ],
        },
        "ameb-musicianship-grade-5-6": {
          title: "AMEB Musicianship Grade 5 级/6 级 高阶课程",
          slug: "ameb-musicianship-grade-5-6",
          content: [
            "AMEB 《Musicianship Grade 5/6》教材",
            "Dulcie Holland 教材作为辅助资料",
            "补充练习与听觉训练资料",
            "每周1次课，每次1小时，共15-20周",
            "乐理、和声、旋律写作与风格分析",
          ],
        },
        "ameb-aural-sight-reading-grade-4-8": {
          title: "AMEB 试唱与视奏课程（乐器/声乐 Grade 4–8）",
          slug: "ameb-aural-sight-reading-grade-4-8",
          content: [
            "AMEB 试唱测试教材（四级至八级）",
            "AMEB 视奏教材（根据所学乐器选择）",
            "钢琴或伴奏录音用于和声训练",
            "总课时：10小时（面对面授课）",
            "小组课或一对一课程",
          ],
        },
        "ameb-general-knowledge-coaching": {
          title:
            "AMEB 音乐常识冲刺课程（适用于 AMEB 8级 / Certificate of Performance / AMusA / LMusA）",
          slug: "ameb-general-knowledge-coaching",
          content: [
            "授课教师：Dr. Sharon Lierse —— 国际知名音乐教育家与学者",
            "一对一授课（可线上或面授，视情况安排）",
            "提供专属学习资料，包括写作模板、术语表、听力参考",
            "灵活课时安排，根据学生需求与考试时间量身定制",
            "针对曲目的深度讲解",
          ],
        },
        "music-appreciation-course": {
          title: "音乐赏析课程",
          slug: "music-appreciation-course",
          content: [
            "主讲教师：Dr Sharon Lierse —— 国际知名音乐教育学者",
            "共计 10节课，每节 60分钟",
            "小组授课，在线教学形式",
            "从巴洛克到20世纪的音乐历史概览",
            "高阶考试写作技巧与术语运用",
          ],
        },
        "vce-music-performance-unit-1-2": {
          title: "VCE 音乐作品演奏 & 当代音乐演奏 Unit 1&2 —— 笔试课程",
          slug: "vce-music-performance-unit-1-2",
          content: [
            "授课教师：Miss Lorraine Zhang",
            "共20次课程，每次60分钟",
            "小班授课/一对一形式，灵活安排",
            "引导学生掌握基础听觉分析、术语、写作表达",
            "为进入Unit 3&4做好基础准备",
          ],
        },
        "vce-music-performance-unit-3-4": {
          title: "VCE 音乐作品演奏 & 当代音乐演奏 Unit 3&4 —— 笔试课程",
          slug: "vce-music-performance-unit-3-4",
          content: [
            "授课教师：Gareth De Korte 先生 —— 墨尔本中学音乐系主任",
            "全年共 20 节课，每节 1 小时",
            "阶段一：SAC 准备阶段（学期 1–3，共 15 节）",
            "阶段二：期末笔试强化阶段（第四学期，共5 节）",
            "演奏导向的试听分析",
          ],
        },
        "ausyouth-music-instrumental-masterclasses": {
          title: "澳升音乐乐器大师课",
          slug: "ausyouth-music-instrumental-masterclasses",
          content: [
            "澳洲顶尖音乐家亲自授课",
            "特邀嘉宾：Wilma Smith（前墨尔本交响乐团首席）和 Rohan De Korte（墨尔本交响乐团大提琴演奏家）",
            "一对一针对性指导，获得个性化反馈",
            "注重表演训练，问答交流与职业分享",
            "曲目与风格精修指导",
          ],
        },
      },
    },
    content: {
      title: "优质的课程内容",
      subtitle: "澳升教育课程内容设计六大核心优势",
      description:
        "澳升教育的课程基于六大核心原则构建：内容全面、深度适宜、目标清晰、循序渐进、实时更新与个性化设计。这些要素相辅相成，既助力学生取得显著的学业进步，又为其未来的成功做好充分准备。",
      features: [
        "内容全面性：覆盖无死角; 课程全面覆盖考试大纲的所有核心知识点，打牢基础；由浅入深，满足不同水平学生的需求；融入跨学科联系和背景知识，拓宽视野，提升综合能力。",
        "内容深度：超越日校; 课程难度远超日校水平，传授高效考试技巧；注重批判性思维、逻辑推理等能力培养；科学递进难度设计，帮助学生稳步提升，全面掌握内容。",
        "科学系统性：结构严谨; 课程循序渐进、逻辑清晰，帮助学生高效掌握知识点；小学到高中的各阶段课程无缝衔接，确保学习连贯；模块化设计灵活实用，方便学生针对性提升。",
        "目标匹配：直指未来需求; 紧扣考试重点和难点，设计高效提分方案；结合竞赛、实践项目，培养学以致用能力；融入升学和职业规划，为学生未来发展提供清晰方向。",
        "实时更新：紧跟趋势; 课程紧随考试政策和大纲调整，内容与时俱进；定期优化教材，结合最新教育趋势；快速响应考试变化，确保学生学习始终聚焦关键内容。",
        "多样需求：个性化为核心; 根据学生的水平和目标，提供个性化课程方案；趣味设计激发学习动力；适当挑战帮助学生突破自我，增强成就感和自信心。",
      ],
      gradeTabs: [
        { value: "y1-y6", label: "Year 1-6" },
        { value: "y7-y9", label: "Year 7-9" },
        { value: "y10-y12", label: "Year 10-12" },
      ],
      gradeContents: [
        {
          value: "y1-y6",
          title: "打牢基础，培养学习兴趣与优秀习惯",
          description:
            "小学阶段是学习的起点，澳升教育注重在这一阶段激发学生的学习兴趣，并培养良好的学习习惯",
          contents: [
            {
              title: "多元趣味课堂",
              description:
                "通过丰富的教学内容和互动式课堂，拓宽学生的知识面，激发学习兴趣，培养他们专注和主动学习的能力。课程设计超于日校、高于日校、深于日校，帮助学生站稳起跑线。",
              icon: "book",
              image: "/courses/content/y1-6-1.jpeg",
            },
            {
              title: "课后巩固",
              description:
                "随堂纸质作业和线上作业系统结合，帮助学生牢牢掌握课堂知识，逐步学会举一反三，提高综合运用能力。",
              icon: "repeat",
              image: "/courses/content/y1-6-2.jpeg",
            },
            {
              title: "阶段性成果检测",
              description:
                "期末测试和考试训练，从小锻炼学生的抗压能力和考试心态，为未来的学习做好心理准备。",
              icon: "check",
              image: "/courses/content/y1-6-3.jpeg",
            },
            {
              title: "考试冲刺+全真模考",
              description:
                "针对NAPLAN、奖学金考试、AEAS、AMC等重要考试，帮助学生掌握应试技巧和时间分配，为考试成绩负责，建立学习信心。",
              icon: "test",
              image: "/courses/content/y1-6-4.jpeg",
            },
            {
              title: "家校互动",
              description:
                "成绩报告与家长会定期举行，帮助家长全面了解孩子的学习状态，紧密配合，优化学习规划。",
              icon: "communication",
              image: "/courses/content/y1-6-5.jpeg",
            },
          ],
        },
        {
          value: "y7-y9",
          title: "强化学习能力，搭建学科框架，为VCE打下坚实基础",
          description:
            "初中阶段是学习从兴趣培养到能力提升的重要过渡期。在这一阶段，澳升教育注重培养学生的学习方法和逻辑思维能力，同时帮助他们在学科知识上打好地基",
          contents: [
            {
              title: "科学衔接的教材体系",
              description:
                "我们的内部教材不仅高于日校水平，还针对初高中衔接的核心内容进行专项设计，确保学生在知识点上无缝过渡。",
              icon: "book",
              image: "/courses/content/y7-9-1.jpeg",
            },
            {
              title: "资深教师指导",
              description:
                "教师团队注重培养学生的学科兴趣，同时强化学科知识点的掌握，为高中复杂学科的学习做好准备。",
              icon: "chalkboard",
              image: "/courses/content/y7-9-2.jpeg",
            },
            {
              title: "系统化作业设计",
              description:
                "课后作业帮助学生深度巩固课堂内容，锻炼解题能力，逐步形成自主学习的习惯。",
              icon: "clipboard-list",
              image: "/courses/content/y7-9-3.jpeg",
            },
            {
              title: "综合能力训练",
              description:
                "通过学期末测试和模拟考试，提升学生的考试技巧、时间分配能力及抗压能力，帮助他们在实际考试中从容发挥。",
              icon: "clock",
              image: "/courses/content/y7-9-4.jpeg",
            },
            {
              title: "重点考试冲刺",
              description:
                "精英公校考试、奖学金考试、NAPLAN、AEAS、AMC等冲刺课程，帮助学生稳步提升成绩，为未来更高目标铺平道路。",
              icon: "award",
              image: "/courses/content/y7-9-5.jpeg",
            },
            {
              title: "早期规划与家校沟通",
              description:
                "通过成绩报告和家长会，密切跟进学生的学习状态，为即将到来的VCE阶段提前做好学科选择和学习规划。",
              icon: "communication",
              image: "/courses/content/y7-9-6.jpeg",
            },
          ],
        },
        {
          value: "y10-y12",
          title: "全力冲刺VCE，定制化教学助力高分实现",
          description:
            "高中阶段是学生学术生涯的关键时期，澳升教育以保证学生的VCE成绩为目标，打造最专业的教学体系",
          contents: [
            {
              title: "精准选课规划",
              description:
                "专业教师根据学生的个人兴趣、学科优势和大学申请目标，制定最优的选课方案，为高分铺平道路。",
              icon: "compass",
              image: "/courses/content/y10-12-1.jpeg",
            },
            {
              title: "独家教纲与教学计划",
              description:
                "结合十年的教学经验，我们精心设计了高效且高分导向的教学进度规划，涵盖核心知识点梳理、多轮复习周期及刷题提分策略。",
              icon: "file-text",
              image: "/courses/content/y10-12-2.jpeg",
            },
            {
              title: "定制化教学方案",
              description:
                "根据学生的学习水平和需求，将课程分为不同难度和进度的班型，提供大班课、小班课、一对一教学等多种选择，帮助学生找到最适合的学习方法和节奏。",
              icon: "sliders-horizontal",
              image: "/courses/content/y10-12-3.jpeg",
            },
            {
              title: "权威教师团队",
              description:
                "VCE领域的顶级教育专家亲自授课，确保课堂内容更深入、更贴近考纲，同时高效覆盖考试必考点，显著提升学习效果。",
              icon: "users",
              image: "/courses/content/y10-12-4.jpeg",
            },
            {
              title: "深度刷题与作业系统",
              description:
                "通过纸质作业与线上系统结合，帮助学生练就快速解题能力，并熟悉所有题型，全面掌握考试技巧。",
              icon: "clipboard-list",
              image: "/courses/content/y10-12-5.jpeg",
            },
            {
              title: "学期末排名测试",
              description:
                "结合澳升十年大数据，帮助学生了解在全维州的大致排名，并及时调整学习策略，为高分目标做足准备。",
              icon: "bar-chart-2",
              image: "/courses/content/y10-12-6.jpeg",
            },
            {
              title: "SAC专项复习",
              description:
                "针对VCE中每一个得分点，提供系统性复习与专项训练，确保学生在学校的考试中表现出色，稳步提升总分。",
              icon: "target",
              image: "/courses/content/y10-12-7.jpeg",
            },
            {
              title: "全真模考与冲刺课程",
              description:
                "全面检测学生对VCE考点的掌握情况，并通过最后阶段的查漏补缺和冲刺班，帮助学生锁定高分。",
              icon: "list-checks",
              image: "/courses/content/y10-12-8.jpeg",
            },
          ],
        },
      ],
    },
    feature: [
      {
        title: "最高标准的教学团队",
        subtitle: "高水平师资，点燃学习兴趣",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "教师资质",
            description:
              "我们拥有全澳最高水平的教师团队，授课老师均具备丰富的教学经验，许多人还拥有顶尖成绩或名校背景。",
            icon: "/courses/icons/team icon 1.svg",
          },
          {
            title: "授课风格",
            description:
              "每位老师都善于将复杂概念讲解得通俗易懂，融入幽默与互动，激发学生学习兴趣，提升课堂参与度。",
            icon: "/courses/icons/team icon 2.svg",
          },
          {
            title: "个性化指导",
            description:
              "我们尊重每位学生的差异，根据他们的需求和进度灵活调整教学内容与节奏，让学习更高效、更贴心。",
            icon: "/courses/icons/team icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-teachers/1.png",
            "/courses/carousel-teachers/2.png",
            "/courses/carousel-teachers/3.png",
            "/courses/carousel-teachers/4.png",
            "/courses/carousel-teachers/5.png",
            "/courses/carousel-teachers/6.png",
            "/courses/carousel-teachers/7.png",
            "/courses/carousel-teachers/8.png",
            "/courses/carousel-teachers/9.png",
            "/courses/carousel-teachers/10.png",
            "/courses/carousel-teachers/11.png",
            "/courses/carousel-teachers/12.png",
            "/courses/carousel-teachers/13.png",
            "/courses/carousel-teachers/14.png",
          ],
        },
      },
      {
        title: "显著的学习效果",
        subtitle: "让进步看得见、摸得着",
        color: "bg-yellow-500/80",
        textColor: "text-yellow-500/80",
        advantages: [
          {
            title: "显著的成绩提升",
            description:
              "我们以数据说话，课程帮助无数学生在VCE、UCAT、奖学金、精英公校、Naplan等关键考试中实现分数突破。",
            icon: "/courses/icons/outcome icon 1.svg",
          },
          {
            title: "能力全面提升",
            description:
              "通过训练解题技巧、时间管理能力、批判性思维等核心技能，帮助学生获得长远竞争力。",
            icon: "/courses/icons/outcome icon 2.svg",
          },
          {
            title: "学习信心倍增",
            description:
              "课程不仅提升成绩，还让学生变得更加自信和积极，对学习充满热情和期待。",
            icon: "/courses/icons/outcome icon 3.svg",
          },
        ],
        button: {
          text: "了解我们的成果",
          link: "/achievements",
          fromColor: "from-yellow-500",
          toColor: "to-yellow-700",
        },
      },
      {
        title: "科学的教学模式",
        subtitle: "系统性与灵活性并存",
        color: "bg-red-700/80",
        textColor: "text-red-700/80",
        advantages: [
          {
            title: "课程设计",
            description:
              "课程设计循序渐进、结构严谨，从基础到高阶，每一步都为学生奠定坚实的知识基础，避免任何知识断层。",
            icon: "/courses/icons/meth icon 1.svg",
          },
          {
            title: "多样化授课形式",
            description:
              "提供线下课堂、线上互动课、混合学习模式，满足不同学习需求，让学生在灵活中保持高效。",
            icon: "/courses/icons/meth icon 2.svg",
          },
          {
            title: "强互动性",
            description:
              "课堂中充分设置提问、互动、练习和反馈环节，确保学生真正理解和内化知识，提升学习体验。",
            icon: "/courses/icons/meth icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-1/1.png",
            "/courses/carousel-1/2.png",
            "/courses/carousel-1/3.png",
            "/courses/carousel-1/4.png",
            "/courses/carousel-1/5.png",
            "/courses/carousel-1/6.png",
            "/courses/carousel-1/7.png",
            "/courses/carousel-1/8.png",
            "/courses/carousel-1/9.png",
            "/courses/carousel-1/10.png",
          ],
        },
      },
      {
        title: "全面的资源支持",
        subtitle: "从教材到测试，助力学习每一步",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "优质内部教材",
            description:
              "我们开发了独家高质量教材，涵盖所有考试重点，并提供丰富的补充学习资料。",
            icon: "/courses/icons/support icon 1.svg",
          },
          {
            title: "作业与练习",
            description:
              "课程配备足量且高质量的课后练习，从巩固知识到提升解题能力，全方位支持学生的学习。",
            icon: "/courses/icons/support icon 2.svg",
          },
          {
            title: "阶段性测试",
            description:
              "定期举行阶段性测评与全真模考，帮助学生及时检测学习成果，调整学习策略，直至稳步提升。",
            icon: "/courses/icons/support icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-2/1.png",
            "/courses/carousel-2/2.png",
            "/courses/carousel-2/3.png",
            "/courses/carousel-2/4.png",
            "/courses/carousel-2/5.png",
            "/courses/carousel-2/6.png",
            "/courses/carousel-2/7.png",
          ],
        },
      },
      {
        title: "学生和家长的高度满意",
        subtitle: "体验高效、收获满满",
        color: "bg-yellow-500/80",
        textColor: "text-yellow-500/80",
        advantages: [
          {
            title: "学生体验优异",
            description:
              "学生普遍反映课程内容有趣、高效，既有学习的成就感，又能享受到知识探索的乐趣。",
            icon: "/courses/icons/sat icon 1.svg",
          },
          {
            title: "紧密的家校互动",
            description:
              "我们通过成绩报告、家长会等形式，实时反馈学生的学习状态，与家长保持紧密沟通，共同优化学习计划。",
            icon: "/courses/icons/sat icon 2.svg",
          },
          {
            title: "优秀的社会评价",
            description:
              "凭借多年成功案例，我们已赢得学生、家长和社会各界的广泛认可和口碑支持。",
            icon: "/courses/icons/sat icon 3.svg",
          },
        ],
        carousel: {
          images: [
            "/courses/carousel-3/1.png",
            "/courses/carousel-3/2.png",
            "/courses/carousel-3/3.png",
            "/courses/carousel-3/4.png",
            "/courses/carousel-3/5.png",
            "/courses/carousel-3/6.png",
            "/courses/carousel-3/7.png",
          ],
        },
      },
      {
        title: "大型机构的全面学习支持体系",
        subtitle: "不仅是课堂，更是全方位服务",
        color: "bg-red-700/80",
        textColor: "text-red-700/80",
        advantages: [
          {
            title: "服务无死角",
            description:
              "从课程规划、学术答疑到心理辅导，我们提供全方位支持，解决学生学习中的所有问题。",
            icon: "/courses/icons/com icon 1.svg",
          },
          {
            title: "灵活且贴心",
            description:
              "根据学生的时间安排和学习目标，我们随时调整课程规划，确保学习效果与效率最大化。",
            icon: "/courses/icons/com icon 2.svg",
          },
          {
            title: "责任保障",
            description:
              "提供合理的退款或补偿政策，用我们的实际行动表达对课程效果的信心和对学生未来的责任感。",
            icon: "/courses/icons/com icon 3.svg",
          },
        ],
        button: {
          text: "了解我们的支持",
          link: "/resource_hub",
          fromColor: "from-red-500",
          toColor: "to-red-700",
        },
      },
      {
        title: "超高性价比",
        subtitle: "资源丰富，价值超越期待",
        color: "bg-primary",
        textColor: "text-primary",
        advantages: [
          {
            title: "丰富资源",
            description:
              "以合理的价格，学生可获得超值的教学内容、丰富的学习资源和全方位支持服务。",
            icon: "/courses/icons/value icon 1.svg",
          },
          {
            title: "附加价值",
            description:
              "我们还提供选课规划、升学指导、学科兴趣培养等增值服务，帮助学生为未来的升学和职业发展做好准备。",
            icon: "/courses/icons/value icon 2.svg",
          },
        ],
        button: {
          text: "了解更多",
          link: "/contact_us",
          fromColor: "from-blue-500",
          toColor: "to-blue-700",
        },
      },
    ],
  },
};
